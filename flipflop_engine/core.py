...
class FlipFlopError(RuntimeError):
    """Raised for FlipFlop engine errors."""
...
def _build_llm_client():
    ...
    if not api_key:
        raise FlipFlopError("Missing OPENAI_API_KEY or AZURE_OPENAI_API_KEY.")
    ...
    if azure_endpoint:
        if AzureOpenAI is None:
            raise FlipFlopError("openai package not installed.")
        return AzureOpenAI(
...
def call_flipflop_llm(noun1: str, noun2: str, *, model: Optional[str] = None) -> str:
    client = _build_llm_client()
    model_id = model or os.getenv("OPENAI_MODEL") or os.getenv("AZURE_OPENAI_DEPLOYMENT") or "gpt-4o-mini"
    messages = [
    ...
    try:
        ...
        raise FlipFlopError(f"LLM call failed: {exc}") from exc

    if not completion.choices:
        raise FlipFlopError("LLM returned no choices.")

    content = completion.choices[0].message.content or ""
    paragraph = content.strip()
    if not paragraph:
        raise FlipFlopError("LLM returned empty content.")
    return paragraph


def enforce_length(paragraph: str, enforce: bool = True) -> Tuple[str, bool]:
    if not enforce:
    ...
    if len(paragraph) < 450:
        raise FlipFlopError("Model returned text shorter than 450 characters.")
    return paragraph, truncated


def maybe_build_image_prompt(paragraph: str, tail: Optional[str]) -> str:
    if tail:
        return f"{paragraph} {tail.strip()}"
    return paragraph


def maybe_generate_image(prompt: str) -> Optional[str]:
    ...
    try:
        ...
        raise FlipFlopError(f"Image backend error: {exc}") from exc
...
def run_flipflop(
    ...
    *,
    ...
    image_style_tail: Optional[str] = None,
    model: Optional[str] = None,
) -> Tuple[str, bool, Optional[str], Optional[str]]:
    paragraph = call_flipflop_llm(noun1.strip(), noun2.strip(), model=model)
    paragraph, truncated = enforce_length(paragraph, enforce_length)

    image_prompt = None
    image_url = None
    if request_image:
        image_prompt = maybe_build_image_prompt(paragraph, image_style_tail)
        image_url = maybe_generate_image(image_prompt)

    return paragraph, truncated, image_prompt, image_url
